...
def get_course_knowledge_base(cname, max_chars=40000):
    ...
    # 0. 강의계획서(Syllabus) 최우선 수집
    syllabus_file = config_manager.get_course_syllabus(folder_name)
    has_syllabus = False
    if syllabus_file and os.path.exists(syllabus_file):
        s_txt = extract_text_from_file(syllabus_file)
        if s_txt:
            has_syllabus = True
            chunk = f"\n\n=== 📑 [공식 강의계획서 (Syllabus) — 마스터 기준] ===\n• 파일명: {os.path.basename(syllabus_file)}\n{s_txt[:15000]}\n"
